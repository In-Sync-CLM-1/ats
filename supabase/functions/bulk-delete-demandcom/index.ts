...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`User ${user.email} (${user.id}) attempting to delete ${recordIds.length} records`);
    ...
    for (const recordId of recordIds) {
      try {
        ...
        await adminSupabase.from('call_logs').delete().eq('demandcom_id', recordId);
        await adminSupabase.from('demandcom_recommendations').delete().eq('demandcom_id', recordId);
        await adminSupabase.from('demandcom_pipeline').delete().eq('demandcom_id', recordId);
        await adminSupabase.from('demandcom_engagement_summary').delete().eq('demandcom_id', recordId);
        await adminSupabase.from('campaign_recipients').delete().eq('demandcom_id', recordId);

        // Delete main record
        const { error } = await adminSupabase.from('demandcom').delete().eq('id', recordId);
        ...
        successCount++;
        console.log(`Successfully deleted demandcom record ${recordId}`);
      } catch (error) {
        errorCount++;
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        errors.push({ recordId, error: errorMessage });
        console.error(`Failed to delete record ${recordId}:`, errorMessage);
      }
    }
