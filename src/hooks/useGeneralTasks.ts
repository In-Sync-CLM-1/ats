...
export function useGeneralTasks(filters?: TaskFilters) {
  ...
  const { data: tasks, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { data: data as GeneralTask[], count: count || 0 };
    },
  });
  ...
  const createTask = useMutation({
    mutationFn: async (taskData: Omit<GeneralTask, "id" | "created_at" | "updated_at" | "completed_at" | "assigned_user" | "assigned_by_user">) => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("Not authenticated");
